...
# -----------------------------------------------------------------------------
# Read MEDIAWIKI_VERSION from .env -> REL format
# -----------------------------------------------------------------------------
mw_rel_version() {
    local ver=""
    [[ -f "$SCRIPT_DIR/.env" ]] && \
...
save_skin_url() {
    ...
    if grep -q "^${skin_name}[[:space:]]" "$SKINS_UPDATES_FILE" 2>/dev/null; then
        ...
        cat "$tmp" > "$SKINS_UPDATES_FILE"; rm -f "$tmp"
    else
...
# -----------------------------------------------------------------------------
# Disable an extension or skin in LocalSettings.php
# -----------------------------------------------------------------------------
disable_in_local_settings() {
    ...
    if grep -q "${load_fn}( '${ext_name}' )" "$LOCAL_SETTINGS" 2>/dev/null; then
        ...
        cat "$tmp" > "$LOCAL_SETTINGS"; rm -f "$tmp"
        ok "${ext_name} disabled in LocalSettings.php."
    fi
}

# -----------------------------------------------------------------------------
# Print Special:Search URL hint for known extension tags
# -----------------------------------------------------------------------------
hint_search_url() {
    local ext_name="$1"
    local pattern=""
    case "$ext_name" in
        EmbedVideo)            pattern="%23ev%3A" ;;
        Graph)                 pattern="%3Cgraph" ;;
        PDFEmbed)              pattern="%3Cpdf" ;;
        Tabs)                  pattern="%3Ctabs" ;;
        SyntaxHighlight_GeSHi) pattern="%3Csyntaxhighlight" ;;
        Scribunto)             pattern="%23invoke%3A" ;;
        Kartographer)          pattern="%3Cmaplink" ;;
        Math)                  pattern="%3Cmath" ;;
        InputBox)              pattern="%3Cinputbox" ;;
        ImageMap)              pattern="%3Cimagemap" ;;
        Poem)                  pattern="%3Cpoem" ;;
        InteractiveTimeline)   pattern="%3Ctimeline" ;;
        ParserFunctions)       pattern="%7B%7B%23if" ;;
    esac
    if [[ -n "$pattern" ]]; then
        local prefix="${WG_SERVER:-}"
        echo "  Check usage: ${prefix}/index.php?title=Special:Search&search=insource%3A${pattern}&ns0=1"
    fi
}

# -----------------------------------------------------------------------------
# Install an extension or skin from a URL (tar.gz or zip)
# -----------------------------------------------------------------------------
install_extension() {
...
# -----------------------------------------------------------------------------
# Prompt and install with auto-download and retry on failure
# -----------------------------------------------------------------------------
prompt_and_install() {
    ...
    while true; do
        ...
        if [[ "$url" == "-" ]]; then
            ...
            read -r -p "  Disable $ext_name in LocalSettings.php? [y/N]: " disable_ext; echo ""
...
# Read $wgServer for URL hints
WG_SERVER=$(grep -m1 "wgServer" "$LOCAL_SETTINGS" 2>/dev/null | grep -o 'https\?://[^"'"'"']*' | head -1 || echo "")
...
# Ensure updates files exist
if [[ ! -f "$UPDATES_FILE" ]]; then
    [[ -f "$UPDATES_EXAMPLE" ]] && cp "$UPDATES_EXAMPLE" "$UPDATES_FILE" || touch "$UPDATES_FILE"
    log "Created $UPDATES_FILE"
fi
if [[ ! -f "$SKINS_UPDATES_FILE" ]]; then
    touch "$SKINS_UPDATES_FILE"
    log "Created $SKINS_UPDATES_FILE"
fi
...
# Build list of extensions, excluding already-skipped ones (unless --reset)
all_extensions=()
for ext_dir in "$EXTENSIONS_DIR"/*/; do
    [[ -d "$ext_dir" ]] || continue
    ext_name=$(basename "$ext_dir")
    _saved=$(saved_url "$ext_name")
    if [[ "$_saved" == "-" ]] && [[ "$RESET" == false ]]; then continue; fi
...
for ext_name in "${all_extensions[@]}"; do
    [[ ! -f "$EXTENSIONS_DIR/$ext_name/version" ]] && no_version+=("$ext_name")
done
...
for ext_name in "${all_extensions[@]}"; do
    [[ -f "$EXTENSIONS_DIR/$ext_name/version" ]] && versioned_extensions+=("$ext_name")
done
...
if [[ -n "$REL_VERSION" ]]; then
    ...
    for ext_name in "${versioned_extensions[@]}"; do
        ext_version=$(head -1 "$EXTENSIONS_DIR/$ext_name/version" | tr -d '[:space:]' | cut -d: -f2)
    ...
    if [[ "${#incompatible[@]}" -gt 0 ]]; then
        ...
        echo ""
        while true; do
            read -r -p "  Your choice [1/2]: " incompat_choice; echo ""
            if [[ "$incompat_choice" == "1" ]]; then
                echo ""
                for ext_name in "${incompatible[@]}"; do
                    disable_in_local_settings "$ext_name"
                    save_url "$ext_name" "-"
                    hint_search_url "$ext_name"
                done
                incompatible=()
                break
                ...
                warn "Invalid choice -- please enter 1 or 2."; echo ""
            fi
        done
    fi
fi
...
if [[ -d "$SKINS_DIR" ]]; then
    ...
    # Phase 1 -- Skins without version file
    no_version_skins=()
    ...
    for entry in "${compatible_skins[@]}"; do
        ...
        prompt_and_install_skin "$skin_name" "$effective_default"
    done
    ...
    for skin_name in "${incompatible_skins[@]}"; do
        ...
        echo -e "${BOLD}--- $skin_name${NC} (current: $skin_version)"
        warn "No package found for $REL_VERSION -- enter a custom URL or skip."
        prompt_and_install_skin "$skin_name"
    done
...
echo ""
hr
echo -e "${BOLD}  Post-restart hints${NC}"
echo ""
echo "  After restarting the container, check for broken pages:"
echo "  LintErrors      : ${WG_SERVER}/index.php?title=Special:LintErrors"
echo "  BrokenRedirects : ${WG_SERVER}/index.php?title=Special:BrokenRedirects"
[[ -z "$WG_SERVER" ]] && echo "  (prefix with your server URL if accessing remotely)"
echo ""
